...
public class Gamification {
    ...
    public List<Mission> getDailyMissions(User user) {
        ...
        if (user.streakDays > 0)
            missions.get(0).completed = true;
        if (user.transactions.size() >= 3)
            missions.get(1).completed = true; // Fix: was user.Transactions

        return missions;
    }

    public void completeMission(User user, String missionId) {
        List<Mission> missions = getDailyMissions(user);

        for (Mission m : missions) {
            if (m.id.equals(missionId) && !m.completed) {
                m.completed = true;
                awardXP(user, m.xpReward, "Mission: " + m.title);
                System.out.println("  ✅ Mission completed: " + m.title);
                break;
            }
        }
    }

    public void rewardSmartSpending(User user) {
        if (user.getSavingsRate() >= 0.1) {
            awardXP(user, 40, "Maintained good savings rate");
        }

        if (!user.isOverBudget("Food")) {
            awardXP(user, 20, "Stayed within Food budget");
        }
    }
